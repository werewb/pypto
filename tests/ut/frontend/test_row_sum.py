"""Dynamic row_sum tests for the @fe.kernel decorator.

Tests the row_sum operation: out[i, 0] = sum_j(tile[i, j])
Used in FlashAttention for softmax normalization (computing sum of exp values per row).
"""
...
def dynamic_row_sum_kernel(
    ...
    tile_row_sum = plm.make_tile(tile_type_row_sum, addr=0x10000, size=256)
    ...
    with pl.section_vector():
        ...
        for i in pl.range(0, M_dim, 64):
            ...
            plm.store(out, tile_row_sum, [i, 0])
...
def test_dynamic_row_sum():
    ...
    for M_val, N_val in shapes:
        ...
        out_ref = a.sum(dim=1, keepdim=True)
        print("***********golden output***********")

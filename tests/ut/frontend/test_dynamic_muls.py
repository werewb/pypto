"""Dynamic muls (scalar multiplication) tests for the @fe.kernel decorator.

Tests the muls operation: out = tile * scalar
Used in FlashAttention for scaling (1/sqrt(head_size)).
"""
...
M = pl.DynVar('M')
N = pl.DynVar('N')
SCALE = 0.125  # 1/sqrt(64) for head_size=64

@fe.kernel
def dynamic_muls_kernel(
    ...
    with pl.section_vector():
        ...
        for i in pl.range(0, M_dim, 64):
            for j in pl.range(0, N_dim, 64):
                ...
                plm.muls(tile_out, tile_a, SCALE)
...
@fe.jit()
def test_dynamic_muls():
    compiled_lib = fe.compile(dynamic_muls_kernel, arch="dav-c220-vec")
